...
class music_chart():
    def __init__(self, key):
        ...
        for j in range(randint(1, 5)):
            pos = randint(0, 9999)
            publisher += str(words[pos])[2:-1] + " "
        self.key = key
        self.frequency_per_year = randint(1, 366)
        self.publisher = publisher
        self.copies = randint(1, 100000)
        self.country = fake.country()
        self.num_of_positions = randint(1, 200)
...
for i in range(LEN_MUSICIANS):
    ...
    experience = randint(p.birth_year + 10, g.year_of_foundation)
...
f = open("data/music_groups.csv", "w")
...
f = open("data/albums.csv", "w")
for album in albums:
    line = "{0},{1},{2}\n".format(album.title, album.release_year, album.copies)
    f.write(line)
f.close()

f = open("data/music_unions.csv", "w")
for union in music_unions:
